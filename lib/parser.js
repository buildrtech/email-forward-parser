...
var REGEXES = {
  ...
  partial_cc_match: [
    ...
    /^.*>,[^<^@^\n]*$/, // '>,' without another '<' or '@' after it
    /^.*=$/, // Ends in '=', indicating that gmail likely split it
    ///^[^@]*$/, // No '@' anywhere on the line, likely just a name
    //^.*[^@]*$/, // No '<', '>' or '@' anywhere on the line, likely just a name
  ],

  cc_with_partial_match: [
    /^\*?\s*Cc\s?:\*?(.+$\n.*)$/m,
  ],
...
class Parser {
  ...
  __parseMailbox(regexes, text, force_array=false) {
    ...
    if (_match && _match.length > 0) {
      ...
      var checkingMatch = true;
      while (checkingMatch) {
        var _partialCCMatch = loopRegexes(
          this.__regexes.partial_cc_match,
          _mailboxesLine.replace(/=\n/, "\n").replace(/\n/g, ""),
        );

        if (_partialCCMatch && _partialCCMatch.length > 0) {
          var _nextLineRegex = new RE2(`(${_mailboxesLine}\\n.*$)`, 'm');
          var _matchAllowingPartial = _nextLineRegex.match(text);
          if (_matchAllowingPartial && _matchAllowingPartial.length > 0) {
            var _matchWithNextLine = _matchAllowingPartial[_matchAllowingPartial.length - 1];
            if (_matchWithNextLine == _mailboxesLine) {
              checkingMatch = false;
            } else {
              _mailboxesLine = _matchWithNextLine;
            }
          } else {
            checkingMatch = false;
          }
      ...
      _mailboxesLine = trimString(_mailboxesLine).replace(/=\n/, "\n").replace(/\n/g, "");
